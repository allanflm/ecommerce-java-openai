...
public class AnaliseDeSentimentos {
    ...
    private static String enviarRequisicao(Path arquivo) throws InterruptedException {
        ...
        var request = ChatCompletionRequest.builder().model("gpt-4-1106-preview").messages(Arrays.asList(new ChatMessage(ChatMessageRole.SYSTEM.value(), promptSistema), new ChatMessage(ChatMessageRole.USER.value(), promptUsuario))).build();
    ...
    private static List<Path> carregarArquivosDeAvaliacoes() {
        try {
            ...
            return Files.walk(diretorioAvaliacoes, 1).filter(path -> path.toString().endsWith(".txt")).collect(Collectors.toList());
        } catch (Exception e) {
    ...
    private static void salvarArquivoDeAnaliseDeSentimento(Path arquivo, String analise) {
        try {
            var nomeProduto = arquivo.getFileName().toString().replace(".txt", "").replace("avaliacoes-", "");
